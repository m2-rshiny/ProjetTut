...
\HRule\\ 
    ...
    {\LARGE\bfseries Développement d'une application RShiny pour comparer des algorithmes d'apprentissage automatique avec caret }\\[0.4cm]  
...
Nous tenons à remercier particulièrement Monsieur Didier Morel pour son aide, sa disponibilité et son écoute tout au long du projet.

Nous remercions aussi son entreprise BD pour la confiance qu'ils nous ont accordés avec ce projet.
...
Dans le cadre de notre préparation au diplôme de master 2 en statistiques et sciences de données, nous avons été amenés à effectuer notre projet tutoré en partenariat avec l'entreprise BD (Becton et Dickinson) sous la tutelle de Didier Morel.
 
Il concerne le développement d'une application RShiny permettant de comparer et de tester des algorithmes de Machine Learning. Dans la mesure où l'entreprise BD ne dispose d'aucune application permettant de réaliser ce type de travail, il semble très intéressant de mettre en place un tel outil. Ainsi, les expérimentés tout comme les personnes sans trop de connaissances en apprentissage automatique pourront s'en servir pour avoir un aperçu de l'efficacité des différentes méthodes.

Dans ce projet, après avoir identifié les besoins et les contraintes, nous avons procédé à la construction de notre application en créant plusieurs fichiers gérant l'interface utilisateur et les traitements côté serveur. Nous avons implémenté dans notre application, des algorithmes de classification supervisée et de régression comme AdaBoost Classification Trees, CART, les k plus proches voisins (kNN), etc... Pour ces différents algorithmes, l'utilisateur a la possibilité de choisir les hyper-paramètres. Nous avons ensuite procédé aux tests sur des jeux de données afin de comparer la performance des différents algorithmes.

# 2. Contexte

L’évolution des techniques de fouilles de données et d'apprentissage automatique, ainsi que l’accroissement des capacités de stockage et de calcul suscite, dans tous les domaines, un intérêt pour l'analyse de données massives. En ce sens, les domaines dans lesquels intervient l'entreprise BD ne font pas exception. Notre projet s'inscrivant dans ce contexte, cette application est destinée à devenir un outil d’investigation rapide utilisable en interne par des collaborateurs BD en possession de jeux de données sur lesquels ils souhaiteraient pouvoir obtenir une idée rapide de la performance de différents types de modèles de prédictions.
...
Le prétraitement des données est une technique utilisée en fouille de données qui consiste à transformer les données brutes dans un format exploitable. Les données réelles sont souvent incomplètes, incohérentes et sont susceptibles de contenir de nombreuses erreurs.
...
- Centrer une variable consiste à soustraire son espérance à chacune de ses valeurs initiales, soit retrancher à chaque valeur la moyenne
...
"One hot encoding" est un processus par lequel les variables catégorielles sont converties en une forme qui pourrait être fournie aux algorithmes d'apprentissage automatique intolérents aux données catégorielles pour faire un meilleur travail de prédiction. Il encode les caractéristiques catégorielles sous la forme de tableau numérique *one-hot*, c'est à dire qu'il transforme une variable catégorielle en un tableau disjonctif complet comportant autant de variables que de modalités.

La fonction *model.matrix* en *R* permet de créer ce tableau disjonctif complet décrit précédemment.

Exemple: on a une variable «couleur» comportant 3 classes *(vert, bleu, noir)* et donc 3 variables binaires sont nécessaires. Une valeur «1» est placée dans la variable binaire pour la couleur correspondante à l'observation, «0» pour les variables des autres couleurs.

### 3.1.2 Imputation des données manquantes

L'imputation de données manquantes est un processus important en fouille de données, il permet d'éviter de se passer de trop d'information. Plusieurs méthodes d'imputation existent.

L'imputation multivariée par équations chaînées */Multivariate imputation by chained equations(MICE)/* est couramment utilisée pour imputer les données manquantes. Nous avons utilisé le package éponyme en R pour implémenter les imputations. Nous avons utilisé les méthodes suivantes :

- rf: imputation par forêt aléatoire
...
- mean : Une autre technique d'imputation qui consiste à remplacer toute valeur manquante par la moyenne de cette variable pour tous les autres cas, ce qui présente l'avantage de ne pas modifier la moyenne de l'échantillon pour cette variable

## 3.2 Echantillonnage - Optimisation du modèle

### 3.2.1 Découpage des données

Afin d'optimiser la capacité de généralisation du modèle, il est important de procéder à l'optimisation des hyper-paramètres. Ceci passe par l'utilisation d’un jeu de validation et l'utilisation de techniques de validation croisée. Au préalable, on définit un jeu de test pour évaluer les performances du modèle *(une fois optimisé)*.

![Découpage train/validation/test](img/valid.png){width=300px}

Avec un jeu de validation on suit ces étapes:
...
Le principe de la validation croisée est le suivant : 

- Découpage de l'échantillon d'apprentissage en k-partitions
- pour chacune des k partitions :
  - l'une d'entre elles sert de données de validation
  - les autres servent de données d'apprentissage
- De manière pratique : pour chacune des $K$ partitions, il faut :
  - mettre de côté la $k-ième$ partition
  - apprendre le modèle sur les partitions restantes
  - tester le modèle sur les données de la $k - ième$ partition

A la fin, on évalue les performances du modèle.

Ainsi, on évalue les performances sur tout le jeu d’apprentissage et on évite le sur-apprentissage

<center>

![Illustration de la validation croisée](img/cv.png)

</center>

## 3.3 Algorithmes d'apprentissage automatique

De nombreux algorithmes d'apprentissage automatiques existent et certains se ressemblent beaucoup. Là est l'intérêt de notre application qui mettra en évidence la différence de performance entre deux agorithmes à priori similaires. Voici quelques exemples d'algorithmes proposés par la librairie caret.

**KNN**

L’algorithme des kNN consiste a trouver les $k$ observations $x_i$ les plus proches de l’observation $x'$ à classifier. Ensuite, il faut definir $f(x')$ en fonction des réponses $y_i$ des kNN. Pour la régression, c’est la valeur moyenne qui est affectée et pour la classification, c'est la classe majoritaire. Les hyper-paramètres de cet algorithme sont le choix du **critere de distance** et de le nombre de **k-voisins**.

- kNN pour la régression:

$$\tilde{f}(x)=moyenne(y_i | i \in N_k(x))$$
$$\Rightarrow  \text{approxime directement la fonction de régression} \ E[Y|X]$$
...
![Barre de navigation](img/navbar.png){width=300px}

</center>

### 4.4.1 Importation des données
...
Les fichiers acceptés sont les fichiers texte avec séparateur. L'utilisateur peut choisir le séparteur, le caractère de citation, préciser s'il y a une ligne d'entête et visualiser les premières lignes du jeu de données importé.

Nous avons décidé d'imposer à l'utilisateur de cliquer sur un bouton pour importer ses données une fois les paramètres d'importation remplis. Nous avons fait ce choix pour éviter à l'application de se mettre à jour dès que l'utilisateur change un paramètre, cela peut prendre beaucoup de temps pour des jeux de données trop volumineux. Ce bouton permet donc de rendre l'onglet plus fluide.

<center>

![Onglet importation des données](img/prepa_data.png)

</center>

### 4.4.2 Préparation et manipulation des données importées
...
![Onglet statistiques descriptives](img/stat_desc.png)

</center>

### 4.4.4 Choix de l'algorithme et de ses hyper-paramètres
...
#### 4.4.4.1 Choix des algorithmes

Notre objectif était ici de permettre à l'utilisateur de choisir de manière simple, un ou plusieur algorithmes à tester sur son jeu de données. Le problème est que la librairie Caret de R propose une énorme quantité d'algorithmes de classification et de régression. Il n'était donc pas pensable d'écrire cette liste en dur dans le code. Nous avons plutôt choisi d'en enregistrer une liste dans un fichier JSON. Ce fichier liste les détails de chaque algorithme(nom, hyper-paramètres, description, etc...). Ainsi, l'administrateur de l'application pourra ajouter ou supprimer des algorithmes de cette liste. Cette liste est celle qui est proposée dans l'interface utilisateur au travers d'un menu déroulant à choix multiple.

#### 4.4.4.2 Paramétrisation des algorithmes
...
Nous avons pu récupérer de manière automatique la liste des hyper-paramètres possibles pour chaque algorithme grâce à la fonction *getModelInfo()* de la librairie caret.
...
#### 4.4.4.3 Sauvegarde/Chargement d'une configuration
...
#### 4.4.4.4 Lien entre la saisie de l'utilisateur et le traitement côté serveur
...
![Onglet choix algorithmes et hyper-paramètres](img/hyper_params.png)

</center>

### 4.4.5 Execution des algorithmes
...
![Onglet performances algorithmes 1/2](img/algos1_2.png)

</center>

<center>

![Onglet performances algorithmes 2/2](img/algos2_2.png)
...
Un des objectifs du projet était de réaliser une application qui soit à la fois *facilement extensible*, et *facilement maintenable*. Le problème est que le code d'une application Shiny peut très vite devenir illisible si l'on reste sur le découpage traditionnel en deux fichiers, c'est-à-dire un fichier `ui.R` pour l'interface graphique et un fichier `server.R` pour le traitement des données côté serveur. D'autres contraintes s'ajoutent également, que l'on peut résumer par les points suivants :

* **Code source difficile à lire** : dans le code, toutes les fonctionnalités de l'application sont mélangées au même endroit, il est alors difficile de retrouver le code correspondant à la fonctionnalité, que ce soit ses éléments graphiques, ou le traitement associé. De même, il peut arriver que le code de traitement côté serveur soit mélangé avec le code pour générer l'interface graphique (par exemple, dans le cas où nous générons une interface réactive). On aimerait donc pouvoir diviser le code en plusieurs modules, afin de regrouper au sein d'une même structure le code associé à chaque fonctionnalité ;
...
* **Aucune contrainte d'accès pour chaque fonctionnalité** : chaque fonctionnalité a accès à toutes les données de l'application, ce qui peut être difficile à déboguer si, par exemple, un élément graphique modifie un comportement du programme auquel il ne devrait pas avoir accès. On aimerait donc pouvoir limiter et contrôler l'accès aux données de chaque fonctionnalité ;
...
Chaque module est ainsi facilement maintenable, réutilisable et modifiable. De plus, chaque module étant indépendant des autres, il n'y a pas de risque qu'une modification dans un module casse le fonctionnement d'un autre. Il est également possible de créer des sous-modules Shiny.
...
![Diagramme structure du projet](img/diagram.png){width=300px}
